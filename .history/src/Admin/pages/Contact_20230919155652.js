...
const Contact = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setTotal(data.contactus?.length);
      setLoading(false);
    } catch {
      setLoading(false);
    }
  ...
  const deleteHandler = async (id) => {
    try {
      const data = await axios.delete(`${Baseurl}api/v1/api/v1/contactUs/${id}`, Auth);
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const postHandler = async (e) => {
      e.preventDefault();
      try {
        const { data } = await axios.post(`${Baseurl}api/v1/contactUs`, payload);
        Store.addNotification({
          title: "",
