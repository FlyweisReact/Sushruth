...
const Privacy = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const { data } = await axios.get(`${Baseurl}api/v1/privacy&policy`);
      setData(data.privacy);
      setTotal(data.privacy?.length);
      setLoading(false);
    } catch {
      setLoading(false);
    }
  ...
  const deleteHandler = async (id) => {
    try {
      const data = await axios.delete(`${Baseurl}api/v1/privacy&policy/add/${id}`, Auth);
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const postHandler = async (e) => {
      e.preventDefault();
      try {
        const { data } = await axios.post(`${Baseurl}api/v1/privacy&policy/add`, {
          terms,
        });
        Store.addNotification({
          title: "",
    ...
    const putHandler = async (e) => {
      e.preventDefault();
      try {
        const { data } = await axios.put(`${Baseurl}api/v1/privacy&policy/add/${id}`, {
          terms,
        });
        Store.addNotification({
          title: "",
